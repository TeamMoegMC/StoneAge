...
public class GenericFluidEntityHandler extends SavedData {
    ...
    public GenericFluidEntityHandler(@Nonnull Level world, @Nonnull final Setup setup) {
        super();
        this.setup = setup;
        this.world = world;
    }
...
//    @Override
    public void load(@Nonnull CompoundTag compoundNBT) {
        ListTag capabilityList = (ListTag)compoundNBT.get("capabilities");
        ...
        capabilityList.forEach(tag -> {
            if (tag instanceof CompoundTag) {
                ...
                ListTag entitiesList = (ListTag)compound.get("entities");
    ...
    private List<LazyOptional<FluidTank>> findNetworksAround(@Nonnull BlockPos pos) {
        return setup.directions.stream().map(pos::relative).filter(ENTITIES::containsKey).map(ENTITIES::get).distinct().collect(Collectors.toList());
    }

    @Nonnull
    private Set<BlockPos> findEntitiesAround(@Nonnull BlockPos pos) {
        return setup.directions.stream().map(pos::relative).filter(ENTITIES::containsKey).collect(Collectors.toSet());
    }
    ...
    private Set<BlockPos> findUniqueEntitiesAround(@Nonnull BlockPos pos, @Nonnull Set<BlockPos> map) {
        return setup.directions.stream()
                .map(dir -> new Pair<>(dir, pos.relative(dir)))
    ...
    public abstract static class Setup {
        ...
        public abstract boolean checkConnection(@Nonnull Level world, @Nonnull BlockPos pos, @Nonnull Direction posFacing, @Nonnull BlockPos pos2);
    }
}
