...
public class KineticEnergyHandler extends SavedData {
    ...
    private static final HashMap<Level, KineticEnergyHandler> INSTANCE = new HashMap<>();

    private KineticEnergyHandler(@Nonnull Level world) {
        super();
        this.world = world;
    }

    public static KineticEnergyHandler getInstance(@Nonnull Level world) {
...
//    @Override
    public void load(@Nonnull CompoundTag tag) {
        CompoundTag genTag = (CompoundTag) tag.get("generators");
        CompoundTag spiTag = (CompoundTag) tag.get("spindles");
        CompoundTag cacheGenTag = (CompoundTag) tag.get("cacheGenerator");
        CompoundTag cacheSpiTag = (CompoundTag) tag.get("cacheSpindle");
    ...
    public CompoundTag save(@Nonnull CompoundTag compound) {
        ...
        CompoundTag spiTag = new CompoundTag();
        CompoundTag cacheGenTag = new CompoundTag();
        CompoundTag cacheSpiTag = new CompoundTag();
    ...
    private void cacheSpindlesForGenerator(@Nonnull BlockPos generatorPosition, @Nonnull IKineticGenerator generator, @Nonnull Direction direction) {
        for (int i = 0; i < 8; i++) {
            BlockPos spindlePosition = generatorPosition.relative(direction, i + 1);
    ...
    private Pair<IKineticGenerator, BlockPos> cacheGeneratorsInDirection(@Nonnull BlockPos position, @Nonnull Direction direction) {
        ...
        for (int i = 0; i < 8; i++) {
            BlockPos generatorPosition = position.relative(direction, i + 1);
            ...
            if (generator != null && hasSameDirection(generator, direction)) {
                ...
                if (8 - distToGenerator - 1 > 0) {
                    for (int j = 0; j < 8 - distToGenerator - 1; j++) {
                        BlockPos spindlePosition = position.relative(direction, -1 - j);
                        IKineticSpindle spindle = getTileEntity(REGISTRY_SPINDLE, spindlePosition);
    ...
    private void uncacheSpindleInDirection(@Nonnull BlockPos position, @Nonnull Direction direction) {
        ...
        for (int i = 0; i < 16; i++) { // find connected generator
            BlockPos tmpPos = position.relative(direction, i + 1);
        ...
        if (hasGenerator && (distToGenerator - 8 > 0)) {
            for (int i = 0; i < distToGenerator - 8; i++) {
                uncacheSpindle(position.relative(direction, i + 1));
            }
        }
    ...
    private <T extends IKinetic> T getTileEntity(@Nonnull HashSet<BlockPos> map, @Nonnull BlockPos position) {
        ...
        if (map.contains(position)) {
            ...
            kinetic = (T) world.getBlockEntity(position);
    ...
    private static void writeMap(@Nonnull CompoundTag tag, @Nonnull HashSet<BlockPos> map) {
    ...
    private static void writeMap(@Nonnull CompoundTag tag, @Nonnull HashMap<BlockPos, BlockPos> map) {
        AtomicInteger index = new AtomicInteger();
    ...
    private static void writeMapMap(@Nonnull CompoundTag tag, @Nonnull HashMap<BlockPos, HashSet<BlockPos>> map) {
        AtomicInteger index = new AtomicInteger();
        map.forEach((position, spindles) -> {
            CompoundTag tmp = new CompoundTag();
            CompoundTag tmpMap = new CompoundTag();
    ...
    private static void readMap(@Nonnull CompoundTag tag, @Nonnull HashSet<BlockPos> map) {
        int index = 0;
        CompoundTag data;

        while ((data = (CompoundTag) tag.get("pos" + index++)) != null) {
            int x = data.getInt("x");
            int y = data.getInt("y");
            int z = data.getInt("z");
    ...
    private static void readMap(@Nonnull CompoundTag tag, @Nonnull HashMap<BlockPos, BlockPos> map) {
        int index = 0;
        CompoundTag data;
    ...
    private static void readMapMap(@Nonnull CompoundTag tag, @Nonnull HashMap<BlockPos, HashSet<BlockPos>> map) {
        int index = 0;
        CompoundTag data;
