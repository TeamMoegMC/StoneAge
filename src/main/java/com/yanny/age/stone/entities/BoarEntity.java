...
public class BoarEntity extends WildAnimalEntity implements TopEntityInfoProvider {
    ...
    public BoarEntity(@Nonnull EntityType<BoarEntity> type, @Nonnull Level worldIn) {
        super(type, worldIn);
    }
    ...
    public AgeableMob getBreedOffspring(@Nonnull ServerLevel serverWorld, @Nonnull AgeableMob ageable) {
        if (Math.min(entityData.get(GENERATION), ageable.getEntityData().get(GENERATION)) >= ConfigHolder.SERVER.domesticateAfterGenerations.get()) {
            EntityType<?> child = ForgeRegistries.ENTITY_TYPES.getValue(new ResourceLocation(ConfigHolder.SERVER.boarBreedingResult.get()));
            ...
            if (child != null) {
                ...
                if (result instanceof AgeableMob) {
                    ...
                    LogUtils.getLogger().warn("'{}' is not instance of Ageable entity! Spawning default PIG entity", ConfigHolder.SERVER.boarBreedingResult);
                }
            } else {
                LogUtils.getLogger().warn("'{}' does not exists! Spawning default PIG entity", ConfigHolder.SERVER.boarBreedingResult);
            }
    ...
    public boolean doHurtTarget(Entity entityIn) {
        ...
        return entityIn.hurt(entityIn.damageSources().mobAttack(this), 4.0F);
    }
